...
description: "웹 프로젝트를 시작할 때 기획을 구체화하고 문서 구조를 세팅하는 스킬. 개발자를 대상으로 기능, 기술 스택, MVP 스코프를 인터뷰로 확정하고 프로젝트 문서 + CLAUDE.md를 생성한다. '웹 프로젝트 시작', '새 프로젝트 시작', '웹앱 기획', 'web project plan', '문서 구조 잡아줘', '프로젝트 세팅', '스택 잡아줘' 같은 말이 나오면 트리거한다."
license: MIT
metadata:
  author: rhino-ty
  version: "1.0.0"
...
새 웹 프로젝트 시작 시, 인터뷰로 기획을 구체화하고 문서 구조 + 파일 생성까지 완료하는 스킬.
...
>
> **5. 기존에 쓰는 스택이나 ��일러플레이트 있어?**
>
> **6. 패키지 매���저?**
> → pnpm / npm / yarn / bun / 아직 모름
...
**프레임워크 선택 기준** (Phase 1 ���변 기반):
- SSR/SSG 필요 + React → **Next.js 15 (App Router)**
- SSR/SSG 필요 + Vue → **Nuxt 3**
- SPA + React → **Vite + React**
- SPA + Vue → **Vite + Vue**
- 정적 콘텐츠 위주 → **Astro**
- 아직 모름 → Next.js 15 기본 제안 (가장 범용적)
...
| ��역          | 선���                        | 이유 |
|---------------|-----------------------------|------|
| Pkg Manager   | pnpm                        | ...  | ← Phase 1-6 답변 반영
| Frontend      | Next.js 15 (App Router)     | ...  | ← 위 기준 적���
...
**"변경하고 싶은 거 있어?"** — confirm 또는 수정 받으�� 스택 확정.
...
| □ | CLAUDE.md에 파일 구조 | 에이전트가 파일 생성 위치를 즉��� 파악 |
...
직��적���고 기술적인 ��화체. Phase별로 끊어서 제시한다.
